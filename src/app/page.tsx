...
const quickActions = [
  { id: 'resume', label: 'Fazer resumo', icon: FileText, color: 'from-blue-500 to-cyan-500' },
  { id: 'translate', label: 'Traduzir', icon: Globe, color: 'from-green-500 to-emerald-500' },
  { id: 'idea', label: 'Gerar ideia', icon: Lightbulb, color: 'from-yellow-500 to-orange-500' },
  { id: 'explain', label: 'Explicar', icon: BookOpen, color: 'from-purple-500 to-pink-500' },
];
  ...
  const Sidebar = () => (
    ...
    >
      ...
      <div className="p-4 space-y-3">
        {[
          { id: 'chat', label: 'Chat', icon: MessageSquare, count: messages.length, color: 'text-cyan-400' },
          { id: 'commands', label: 'Comandos', icon: Command, color: 'text-blue-400' },
          ...(currentMode === 'pro' ? [{ id: 'agents', label: 'Agentes', icon: Users, color: 'text-purple-400' }] : []),
          { id: 'settings', label: 'Configurações', icon: Settings, color: 'text-orange-400' },
        ].map((item, index) => {
          ...
          return (
            ...
            >
              ...
              >
                <Icon className={`w-5 h-5 ${isActive ? 'text-cyan-400' : item.color}`} />
                  ...
                  >
                    <Card className="glass-card hover-lift focus-ring cursor-pointer border-white/20 hover:border-cyan-500/50 transition-all duration-300 group">
                      <CardContent className="p-6 text-center">
                        <div className={`w-12 h-12 mx-auto mb-4 rounded-xl bg-gradient-to-r ${action.color} flex items-center justify-center shadow-glow group-hover:shadow-glow-strong transition-all duration-300`}>
